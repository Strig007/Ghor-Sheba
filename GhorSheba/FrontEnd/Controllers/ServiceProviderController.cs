...
namespace FrontEnd.Controllers
{
    ...
    public class ServiceProviderController : Controller
    {
        ...
        {
            var sb = new BookingModel();

            string token = Session["token"].ToString();
            GlobalVariables.WebApiClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(token);
            HttpResponseMessage response = GlobalVariables.WebApiClient.GetAsync("api/ServiceProvider/EditBooking/" + id.ToString()).Result;
            if (response.IsSuccessStatusCode)
            {
                var ServiceProviderResponse = response.Content.ReadAsStringAsync().Result;
                sb = JsonConvert.DeserializeObject<BookingModel>(ServiceProviderResponse);
            }
            return View(sb);
        }
        [HttpPost]
        public ActionResult EditBooking(BookingModel b)
        {
            ...
            string token = Session["token"].ToString();
            GlobalVariables.WebApiClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(token);
            HttpResponseMessage response = GlobalVariables.WebApiClient.PostAsJsonAsync("api/ServiceProvider/EditBooking", b).Result;
            if(response.IsSuccessStatusCode)
            {
                return RedirectToAction("ViewBooking", "ServiceProvider");
            }
            return View(b);
        }
        ...
        public ActionResult ViewSalary(ServiceProviderModel u)
        {
            int u_id = Int32.Parse(Session["u_id"].ToString());
            string token = Session["token"].ToString();
            GlobalVariables.WebApiClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(token);
            HttpResponseMessage response = GlobalVariables.WebApiClient.GetAsync("api/ServiceProvider/GetSalary/"+u_id.ToString()).Result;

            var sal = new List<UserSalariesModel>();

            if(response.IsSuccessStatusCode)
            {
                var ServiceProviderResponse = response.Content.ReadAsStringAsync().Result;
                var x = JsonConvert.DeserializeObject<UserSalariesModel>(ServiceProviderResponse);
                sal.Add(x);
            }
